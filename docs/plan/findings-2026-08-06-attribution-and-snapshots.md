...
## 근원 수정 — 하류 보상재를 걷어낸 기록 (2026-08-06 오후)

사용자 지적으로 방향이 바뀌었다: *수집부터 틀렸는데 억지로 살리려고 하류를 고치고
있는가, 아까워서 새로 만들 것을 안 만들고 있는가.* 재보니 둘 다였다.

### 오늘 밤 만든 것 셋이 전부 보상재였다

| 근원의 결함 | 만들었던 보상재 |
| --- | --- |
| 추출기가 `object_entity_id` 를 한 번도 안 씀 (011 부터 컬럼 존재) | `run-claim-merge` 가 객체를 텍스트로 묶음 |
| `knowledge.claim` 에 유니크 제약 없음 (70개 마이그레이션 전수) | 같은 잡이 사후 병합 |
| 링크는 제목만, 추출은 제목+요약을 읽음 | `run-event-text-attribution` 이 요약을 다시 훑음 |

### 고친 것과 실제 수치

```
1단계  링크 표면을 추출 표면에 맞춤     뉴스 링크 672 → 883 (13.7% → 17.8%)
2단계  추출기가 객체 해소 + 모호 거부    object_entity_id 0/325 에서 출발
3단계  claim dedupe 키 (문서 범위)      같은 문서 중복 34건을 막는다
```

### 계획이 틀렸던 곳 셋, 전부 측정이 잡았다

- **분모**: "34%" 는 추출되지 않는 문서 종류까지 섞은 수였다. 추출 대상인 뉴스만
  보면 13.7% 다. 셋 중 둘이 아니라 여덟 중 일곱
- **모호성 가드 위치**: `document_entity` 는 다대다라 하류의 `entity_matches > 1`
  거부를 여기 옮기면 정상 동작(한 기사에 두 회사)을 막는다. `resolveMention` 이
  맞는 자리였다
- **병합 스테이지 제거**: 중복이 두 종류다 — 같은 문서 34건(제약이 잡는다)과 문서
  간 10건(7일 창이 필요해 제약으로 표현 불가). 병합 잡은 후자를 담당하므로 유지

### 만들었다가 걷어낸 것

캐노니컬 이름 링커. 단독 770 문서였는데 넓힌 별칭 뒤에 돌리니 **4건**이었다 —
표면이 같아지면 두 카탈로그가 겹친다. 마이그레이션 070 은 적용된 채 두고(적용된
마이그레이션을 되돌리는 것이 안 쓰이는 허용값보다 나쁘다) 4라는 수치를 남겼다.

### 내가 파이프라인을 세 번 깨뜨렸다

```
14:45 · 14:55 · 15:04   knowledge-wrapper failed
사유  document_entity_link_method_check 위반
```

캐노니컬 링커를 워킹 트리에 둔 채 마이그레이션 070 을 적용하기 전이었다. **타이머는
커밋이 아니라 워킹 트리를 실행한다** — 위험을 식별하고 확인을 기다리는 동안 이미
라이브였다. 순서가 거꾸로였고, 마이그레이션을 먼저 적용하고 코드를 고쳤어야 했다.
15:12 에 070 적용, 15:15 에 링커 제거, 15:4x 실행에서 복구 확인(링크 277건 삽입).

**교훈은 "커밋 전이라 안전하다" 가 거짓이라는 것이다.** 이 저장소에서 편집은 곧
배포다.

### 4단계는 계획보다 싸게 닫는다

269 문서를 강제 재추출하려 했으나, 대기 문서 385건이 다음 실행에서 새 추출기를
자연히 통과한다. 새것(객체 엔티티·dedupe 키 보유) 대 옛것(327건 텍스트) 비교가
**LLM 비용 없이** 생긴다. 그 수치를 보고 옛것 처리를 정한다.
