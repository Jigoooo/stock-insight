...
### 8-1. Market Home 시장 지표 — 완료 (2026-08-13)

`sampleMarketIndicators`(KOSPI·NASDAQ·금)를 정본 01 §2 네 축의 실데이터로
교체했다. 그 상수는 `dataState: 'sample'` 로 정직하게 표시된 레이아웃
스캐폴드였고 오류가 아니었다 — 정직성 기계는 유지한 채 행만 바꿨다.

> **이 절은 한 번 틀렸다.** 원래 "**FX 와 원자재는 수집 자체가 없다** … 수집기
> 신규 개발이 필요하다. K7 배선이 아니라 별도 기능이다" 라고 적혀 있었다.
> 2026-08-13 실측으로 반증됐다:
>
> | 계열 | 값 | 최신 관측 |
> | --- | --- | --- |
> | `fred:DEXKOUS` 원/달러 | 1409.94 | 2026-08-07 |
> | `fred:DCOILWTICO` WTI | 81.96 | 2026-08-03 |
> | `fred:DHHNGSP` 천연가스 | 2.81 | 2026-08-03 |
>
> `market.macro_vintage` 21계열 77,248관측이 있고,
> `analytics.macro_series_topic` 이 20계열에 topic 과 한국어 이름을 붙여 뒀으며
> 두 표 모두 `stock_insight_app_reader` 에 SELECT 가 열려 있었다. 필요한 것은
> 수집기가 아니라 **배선**이었다.
>
> 이 오판이 비쌌던 이유는 "별도 기능" 이라는 판정이 그 섹션을 K7 범위 밖으로
> 밀어냈기 때문이다. 문서가 없다고 적으면 아무도 다시 세지 않는다.

배선된 6행은 정본 §2 네 축을 덮는다: 미국 10년물·국고채(10년)·원/달러·WTI·
천연가스·한국은행 기준금리. 신선도는 단일 임계가 아니라 **발표 주기별**로
판정하고, 기준금리는 다음 금통위까지 유효하므로 stale 판정을 하지 않는다.
