...
>
> ⚠️ **부록 A 는 2026-07-20 기준이며 이미 낡았다.** 지금 실제로 어떻게 지어져 있는지는
> [`stock-insight-as-built-2026-08-07.md`](./stock-insight-as-built-2026-08-07.md) 를 보라 —
> 코드·DB 를 직접 잰 as-built 문서다. **목표가 바뀌면 이 문서를, 현실이 궁금하면 그 문서를** 읽어라.
>
> - 작성일: 2026-07-20 (KST)
> - 기준 문서: `docs/architecture/stock-crypto-insight-platform-architecture.md` (Phase 0~5 기준선)
...
> - **개정 1 (2026-07-20, 구속력 있음)**: 본 문서는 `docs/architecture/stock-insight-v2-enhancement-plan.md` §27의 수정 26건을 전건 수용한다. 본문과 아래 개정 조항이 충돌하면 **개정 조항이 우선**한다. 명칭·계약 동결은 `docs/architecture/adr/ADR-001-v2-naming-freeze.md`를 따른다.
...
*본 문서는 `docs/architecture/stock-crypto-insight-platform-architecture.md`(기준선)와 승인된 complete-v2 로드맵을 통합한 레이어 정본이며, 구현 진행에 따라 부록 A만 갱신한다. 본문 계약 변경은 오탈자 수정을 제외하고 승인 대상이다.*
