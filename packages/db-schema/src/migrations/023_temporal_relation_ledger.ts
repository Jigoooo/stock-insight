...
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.revision_status='accepted' AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'accepted relation revision requires immutable relation evidence';
  END IF;
  RETURN NEW;
END $$;
...
INSERT INTO knowledge.relation_evidence_ledger (
  ...
  evidence_text,evidence_hash,source_weight,recorded_at,metadata
)
...
INSERT INTO knowledge.relation_evidence_ledger (
  ...
  evidence_text,evidence_hash,source_weight,valid_from,recorded_at,metadata
)
...
JOIN knowledge.predicate_ontology_revision ontology
  ON ontology.predicate=relation.predicate AND ontology.revision_no=1
