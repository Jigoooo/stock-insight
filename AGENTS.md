...
- Schema migrations have a ledger: `public.schema_migration`, driven by `pnpm --filter @stock-insight/api schema:status|schema:apply`. It records a checksum per migration and refuses to run when an applied migration's SQL has since been edited. `public.migration_runs` is a *pipeline job* log despite the name — not a schema ledger.
- **The schema cannot be rebuilt from this repository.** The 54 migrations are additive on a base schema owned by the older research app sharing this database (`public.entities`, `public.user_positions`, `stock.*`, `market_ts.ohlcv`, …). A fresh database fails on the first migration. Recovery is backup/restore, not replay: `docs/operations/schema-migration-ledger.md`.
