...
export async function appendRelationEvidence(
  client: PoolClient,
  input:{relationIdentityId:number;sourceContractRevisionId:number;evidenceText:string;evidenceHash:string;sourceWeight?:number},
):Promise<boolean>{
  ...
  const result=await client.query(`
    ...
    ) VALUES ($1,'source_contract',$2,$3,$4,$5,'{"writer":"relation-store"}')
    ON CONFLICT (relation_identity_id,evidence_hash) DO NOTHING
    RETURNING relation_evidence_ledger_id
  `,[input.relationIdentityId,input.sourceContractRevisionId,input.evidenceText,input.evidenceHash,input.sourceWeight??null]);
...
SELECT DISTINCT ON (identity.relation_identity_id)
       ...
       revision.relation_revision_id,revision.revision_no,revision.revision_status,
