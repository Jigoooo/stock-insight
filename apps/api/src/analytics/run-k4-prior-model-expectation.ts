...
import {
  executeK4PriorModelExpectationJob,
  parseK4PriorModelExpectationArgs,
} from './k4-prior-model-expectation-writer.ts';
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  const args = parseK4PriorModelExpectationArgs(argv);
  const Pool = (pg as PgModule).Pool;
  const pool = new Pool({ connectionString: databaseUrl(), max: 1 });
  const client = await pool.connect();
  try {
    const summaries = await executeK4PriorModelExpectationJob({
