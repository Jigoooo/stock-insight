 ...
 * ## 오늘 이 산출물을 읽는 화면은 없다
 *
 * `serving.market_anomaly_current_v1` 은 Market Home 의 해당 섹션이 만들어지면
 * 그것이 읽을 자리다. 섹션은 아직 없다. 같은 날 대시보드 엔드포인트에서
 * "고쳤는데 읽는 곳이 없다" 를 한 번 겪었으므로 여기서는 처음부터 적어둔다 —
 * 소비자가 생기기 전까지 `run-table-reachability-audit` 이 이 표를 "채우는데
 * 아무도 안 읽는" 쪽으로 세는 것은 회귀가 아니라 정확한 보고다.
 */

/**
 * 정본 §9 가 요구하는 탐색 채널 중 이 저장소가 시점에 묶어 뒤질 수 있는 것.
 *
 * §9 의 흐름은 다섯 칸이다: anomaly → known event → exposure → **competing
 * hypotheses** → 상태. 네 번째 칸은 **의도적으로 수행하지 않는다.** 우리가 가진
 * 것은 채널이 무엇을 돌려줬는지뿐이고, 거기서 가설을 만들어내는 것은 정본 06 §9
 * 의 NO-GO(LLM 이 빈 데이터 필드를 서술로 보충)와 REQ-MKT-001 이 함께 막는
 * 바로 그 행동이다. 경쟁 가설은 사람이 채널 결과를 보고 세우는 것이고, 이 표는
 * 그 재료를 정직하게 놓아두는 데까지가 자기 몫이다.
 */
