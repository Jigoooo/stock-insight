...
describe('B0 report truth publication race guard', () => {
  it(
    'rejects unverified facts and removes the latest pointer after retraction',
    { skip: skipReason },
    async () => {
  ...
  it(
    'serializes publish against concurrent retraction and removes the committed pointer',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
