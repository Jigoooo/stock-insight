...
describe('B3 security/issuer identity contract', () => {
  it(
    'maps every Stock to exactly one Company and exposes one ISSUED_BY edge',
    { skip: skipReason },
    async () => {
  ...
  it(
    'identity is entity-ID based and immutable (ticker text is not part of the mapping key)',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      const pool = new pg.Pool({ connectionString: databaseUrl, max: 1 });
      ...
      try {
        const columns = await client.query(
          `SELECT column_name FROM information_schema.columns WHERE table_schema='core' AND table_name='security_issuer_identity'`,
        );
        assert.equal(
          columns.rows.some((row) => row.column_name.includes('ticker')),
          false,
        );
        ...
        await assert.rejects(
          () =>
            client.query(
              `UPDATE core.security_issuer_identity SET mapping_basis='ticker_mutation' WHERE security_issuer_identity_id=(SELECT min(security_issuer_identity_id) FROM core.security_issuer_identity)`,
            ),
          /append-only/,
        );
