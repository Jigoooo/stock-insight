...
async function seedOneDelivery(
  ...
  if (maxAttempts !== 8) {
    await client.query('UPDATE ops.outbox_delivery SET max_attempts = $2 WHERE event_id = $1', [
      inserted.eventId,
      maxAttempts,
    ]);
  }
...
describe('B1 outbox crash recovery and lease fencing', () => {
  it(
    'send-before-ACK crash: expired lease is reclaimed with the SAME identity and a fresh token',
    { skip: skipReason },
    async () => {
  ...
  it(
    'two claimers race: exactly one worker owns a due delivery',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      ...
      try {
        ...
        const [claimsA, claimsB] = await Promise.all([
          claimDueDeliveries(clientA, {
            destination: seeded.destination,
            limit: 1,
            leaseSeconds: 60,
          }),
          claimDueDeliveries(clientB, {
            destination: seeded.destination,
            limit: 1,
            leaseSeconds: 60,
          }),
  ...
  it(
    'bounded retries: exhausted delivery flips to dead and appends exactly one dead letter',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      const pool = new pg.Pool({ connectionString: databaseUrl, max: 1 });
      ...
      try {
        ...
        assert.equal(first.length, 1);
        await client.query('BEGIN');
        const afterFirst = await failDelivery(
          client,
          first[0]!.deliveryId,
          first[0]!.leaseToken,
          'boom-1',
        ...
        assert.equal(second.length, 1);
        await client.query('BEGIN');
        const afterSecond = await failDelivery(
          client,
          second[0]!.deliveryId,
          second[0]!.leaseToken,
          'boom-2',
        ...
        const third = await claimDueDeliveries(client, {
          destination: seeded.destination,
          limit: 1,
          leaseSeconds: 60,
        });
        ...
        assert.equal(
          await failDelivery(client, second[0]!.deliveryId, 'stale-token', 'late', 0),
          'lost_lease',
        );
      } finally {
        client.release();
