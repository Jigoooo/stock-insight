...
describe('B4 verification state machine', () => {
  it(
    'rejects direct verified and one-document verified; accepts two-document path with audit',
    { skip: skipReason },
    async () => {
      ...
      `,
        ...
        `,
              [
                ...
                chunks.rows[1]!.chunk_id,
                String(chunks.rows[1]!.content).slice(0, 200),
              ],
            ),
      ...
      `,
          [
            eventId,
            chunks.rows[0]!.document_id,
            chunks.rows[0]!.chunk_id,
            String(chunks.rows[0]!.content).slice(0, 200),
          ],
        );
        ...
        await assert.rejects(
          () =>
            transitionVerification(client, {
              subject: 'event',
              subjectId: eventId,
              toStatus: 'verified',
              actor: 'b4-test',
              reason: 'direct transition must fail',
            }),
        ...
        assert.equal(
          await transitionVerification(client, {
            subject: 'event',
            subjectId: eventId,
            toStatus: 'corroborated',
            actor: 'b4-test',
            reason: 'one anchored source',
          }),
          true,
        );
        ...
        await assert.rejects(
          () =>
            transitionVerification(client, {
              ...
              toStatus: 'verified',
              actor: 'b4-test',
              reason: 'one source is insufficient',
            }),
      ...
      `,
        ...
        assert.equal(
          await transitionVerification(client, {
            ...
            toStatus: 'verified',
            actor: 'b4-test',
            reason: 'two distinct anchored sources',
          }),
          true,
        );
      ...
      `,
          [
            eventId,
            chunks.rows[2]!.document_id,
            chunks.rows[2]!.chunk_id,
            String(chunks.rows[2]!.content).slice(0, 200),
          ],
        );
        const downgraded = await client.query(
          'SELECT verification_status FROM knowledge.event WHERE event_id=$1',
          [eventId],
        );
      ...
      `,
        ...
        assert.deepEqual(
          audit.rows.map((row) => `${row.from_status}->${row.to_status}:${row.distinct_documents}`),
  ...
  it('requires actor and reason before any transition request', async () => {
    ...
    await assert.rejects(
      () =>
        transitionVerification(fakeClient, {
          subject: 'claim',
          subjectId: 1,
          toStatus: 'corroborated',
          actor: '',
