...
describe('B4 versioned chunk and evidence integrity', () => {
  it(
    'chunks every available legacy title+summary without pretending it is full body',
    { skip: skipReason },
    async () => {
  ...
  it(
    'all corroborated/verified knowledge has the configured distinct docs and chunk quotes',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      const pool = new pg.Pool({ connectionString: databaseUrl, max: 1 });
