...
describe('B1 consumer inbox atomicity', () => {
  it(
    'projection failure rolls the inbox marker back; success is exactly-once per consumer',
    { skip: skipReason },
    async () => {
      ...
      try {
        await client.query(
          `CREATE TABLE IF NOT EXISTS ops.b1_test_projection (event_id TEXT NOT NULL, consumer_id TEXT NOT NULL)`,
        );
