...
describe('B2 raw object + source revision atomicity', () => {
  it(
    'rolls back fetch run, raw row and source revision together',
    { skip: skipReason },
    async () => {
  ...
  it(
    'reuses exact raw bytes and returns a source-revision replay for the same provider key',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      const pool = new pg.Pool({ connectionString: databaseUrl, max: 1 });
      ...
      try {
        ...
        assert.equal(typeof first.rawObjectId, 'number');
  ...
  it(
    'rolls the domain mutation back when an aggregate slot contains the wrong source payload',
    { skip: skipReason },
    async () => {
      assert.ok(databaseUrl);
      const pool = new pg.Pool({ connectionString: databaseUrl, max: 1 });
      ...
      `,
          [
            `evt-conflict-${Date.now()}`,
            String(identity.rows[0]!.source_record_identity_id),
            String(opened.rows[0]!.source_id),
            '0'.repeat(64),
          ],
        );
