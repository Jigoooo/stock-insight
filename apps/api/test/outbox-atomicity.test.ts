...
describe('B1 outbox atomicity', () => {
  ...
  it('domain mutation and outbox insert roll back together', { skip: skipReason }, async () => {
    await withClient(async (client) => {
      const aggregateId = `atomicity-rollback-${randomUUID()}`;
      await client.query('CREATE TABLE IF NOT EXISTS ops.b1_test_domain (id TEXT PRIMARY KEY)');
      await client.query('BEGIN');
      await client.query('INSERT INTO ops.b1_test_domain (id) VALUES ($1)', [aggregateId]);
      const result = await insertOutboxEvent(client, envelopeFixture(aggregateId));
