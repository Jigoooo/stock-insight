...
async function requireScopedReadDatabase(userId: string) {
  const { createScopedReadOnlyDatabaseClient } = await import('@stock-insight/api');
...
async function requireScopedWriteDatabase(userId: string) {
  const { createScopedDatabaseClient } = await import('@stock-insight/api');
...
export async function loadAdminCapabilitiesForUser(userId: string): Promise<AdminCapabilities> {
  const database = await requireScopedReadDatabase(userId);
...
export async function listAdminInvitationsForUser(userId: string): Promise<AdminInvitation[]> {
  const database = await requireScopedReadDatabase(userId);
  return listAdminInvitations(database.queryRows);
}
...
export async function issueAdminInvitationForUser(
  ...
  const database = await requireScopedWriteDatabase(userId);
  return database.withTransaction((executor) => issueAdminInvitation(executor.queryRows, input));
}
...
export async function revokeAdminInvitationForUser(
  ...
  return database.withTransaction((executor) =>
    revokeAdminInvitation(executor.queryRows, invitationId, reason),
  );
}
