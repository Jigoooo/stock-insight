...
describe('public root startup boundary', () => {
  ...
  it('keeps public auth on canonical shared control boundaries', async () => {
  ...
  it('keeps workspace initial and local error fallbacks free of the side-effectful primitive barrel', async () => {
    ...
    assert.match(workspaceLayout, /shared\/ui\/button/);
    assert.match(workspaceLayout, /shared\/ui\/feedback/);
    assert.match(workspaceLayout, /<ErrorState\b/);
    assert.match(workspaceRoute, /viewLoadError/);
  });

  it('keeps public auth control states in canonical component styles', async () => {
    const [button, buttonCss, input, inputGroup, inputCss, link, authCss] = await Promise.all([
      read(buttonUrl),
      read(buttonCssUrl),
      read(inputUrl),
      read(inputGroupUrl),
    ...
    assert.match(buttonCss, /\.button:focus-visible/);
