...
const marketGraphiteProfileUrl = new URL(
  '../public/styles/profiles/market-graphite.css',
  import.meta.url,
);
...
describe('whole-product UX constitution', () => {
  ...
  it('keeps ambiguous blue literals out of the Market Graphite profile', async () => {
    const profile = (await readFile(marketGraphiteProfileUrl, 'utf8')).toLowerCase();

    for (const blue of ['#356faf', '#7fb0eb', '#07101e', '#0e1724']) {
      assert.doesNotMatch(profile, new RegExp(blue));
    }
  });
});
